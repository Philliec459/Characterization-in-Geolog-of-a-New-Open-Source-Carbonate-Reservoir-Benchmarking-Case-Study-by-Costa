...
We received the log data as 17 las files with a text file of log header data including X,Y locations, KB elevation and TD.  There were no directional survey data provided. It is assumed that these wells are all vertical wells, and TVDss was calculated using the KB Elevations for each well. Dr. Costa has confirmed that this is true. 
...
4) Using the kNN or Facimage estimated permeability and calculated Total Porosity (PHIT) from our log analysis, we queried Costa's core database to predict the following Petrophysical properties:
    ...
    - Estimate the most dominant pore throat diameter at each level in each well calculated directly from the Thomeer parameters G and Pd using the Buiting equation shown below. This exact mode of the Pore Throat Distribution (PTD) is calculated using the Buiting equation:

			Mode of PTD (microns) = exp(-1.15 * G1) * (214/Pd1)

This exact Mode of the PTD is what Winland was trying to accomplish with his r35 or Amaefule with FZI; however, both techniques are not exact. We have another GitHub repository under Phillips459 that shows a comparison of these rock typing techniques that should be viewed to see an actual Rock Type comparisons for the different methods using the Mode of the PTD calculated from Ed Clerke's Arab D Rosetta Stone carbonate core dataset with Thomeer Parameters. 
